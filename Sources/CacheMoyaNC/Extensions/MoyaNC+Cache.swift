...
extension MoyaNC {
    ...
    @discardableResult
    internal func cacheProviderRequest<Value: Codable>(_ target: MoyaCacheTarget, _ completion: @escaping Completion<Value>) -> Request {
        return request(target as BaseTargetType) { (result: Result<Value>) in
            switch result {
            case let .success(value): self.storeCache(value: value, for: target)
            case .failure: break
            }
            completion(result)
        }
    ...
    internal func storeCache<Value: Codable>(value: Value, for target: MoyaCacheTarget) {
        guard target.cachePolicy != .notUseCache else { return }
